...
double  ShuntingYard::Pi = 3.14159265359;

std::vector<std::vector<std::string>> ShuntingYard::priorityVector({{"^"}, {"*", "/"}, {"+", "-"}});

std::vector<std::string> ShuntingYard::functionsVector({"sin", "cos", "asin", "acos", "tg", "atg", "ctg", "ln"});

std::vector<std::string> ShuntingYard::unaryVector({"sin", "cos", "asin", "acos", "tg", "atg", "ctg", "ln"});

std::vector<std::string> ShuntingYard::rightAssociativityVector({"^"});
...
double ShuntingYard::fabs(double x) {
    if(x < 0) {
        return -x;
    }
    else {
...
double ShuntingYard::ln(double x) {
    if(x <= 0) {
        throw std::invalid_argument("Incorrect value in ln");
    }
    int i = 2;
...
double ShuntingYard::pow(double a, double b) {
    int i = 1;
    double cur = 1;
    double acc = 1;
    double fact= 1;
...
double ShuntingYard::sin(double x){
    int i = 1;
    double cur = x;
    double acc = 1;
    double fact= 1;
...
double ShuntingYard::cos(double x) {
    return sin(2 * x) / (2 * sin(x));
}



double ShuntingYard::asin(double x) {
    if(fabs(x) > 1) {
...
double ShuntingYard::acos(double x) {
    if(fabs(x) > 1) {
...
double ShuntingYard::tg(double x) {
    if(cos(x) == 0) {
...
double ShuntingYard::ctg(double x) {
    if(sin(x) == 0) {
...
double ShuntingYard::atg(double x){
    return asin(x / pow(1 + pow(x, 1/2), 1/2)); 
}


bool ShuntingYard::isNumber(std::string token)  {
    int count = 0;
    if (token.empty()) {
        return false;
    }
...
bool ShuntingYard::isFunction(const std::string& token) {
...
bool ShuntingYard::isOperator(const std::string& token) {
...
ptrdiff_t ShuntingYard::priority(const std::string& token) {
...
ShuntingYard::Associativity ShuntingYard::associativity(const std::string& token) {
    if (token == "^") {
...
ShuntingYard::Unarity ShuntingYard::unarity(const std::string& token) {
...
double ShuntingYard::unary(const std::string& operation, double operand) {
    if (operation == "sin") {
        return sin(operand);
    }
...
double ShuntingYard::binary(const std::string& operation, double operand1, double operand2) {
    if (operation == "+") {
